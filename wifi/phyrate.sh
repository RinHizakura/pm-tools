#!/usr/bin/env bash

# Computes the theoretical 802.11 PHY rate for a WiFi interface's current
# connection, across legacy (a/b/g), HT (n), VHT (ac), HE (ax) and EHT (be).
#
#   rate_Mbps = NSS * (Nsd * bits_per_subcarrier * code_rate) / Tsym_us
#
# Usage: phyrate.sh <iface>
#   The interface must be currently associated to an AP/hotspot.

# bits-per-subcarrier * code-rate, indexed by MCS 0-13.
# 0-9 shared by HT/VHT/HE/EHT, 10-11 add HE/EHT 1024-QAM, 12-13 add EHT 4096-QAM.
BPSCR=(0.5 1.0 1.5 2.0 3.0 4.0 4.5 5.0 6.0 6.666667 7.5 8.333333 9.0 10.0)
# BPSCR[mcs] = MOD_BITS[mcs] (bits/symbol, i.e. log2(modulation order)) * CR_LABEL[mcs] (FEC code rate)
MOD_NAME=(BPSK QPSK QPSK 16-QAM 16-QAM 64-QAM 64-QAM 64-QAM 256-QAM 256-QAM 1024-QAM 1024-QAM 4096-QAM 4096-QAM)
MOD_BITS=(1 2 2 4 4 6 6 6 8 8 10 10 12 12)
CR_LABEL=("1/2" "1/2" "3/4" "1/2" "3/4" "2/3" "3/4" "5/6" "3/4" "5/6" "3/4" "5/6" "3/4" "5/6")

declare -A nsd_table tsym_table

# set_gen_profile GEN -> fills nsd_table/tsym_table/width_options/gi_options for
# this generation. HT/VHT use a 64/128/256/512-point FFT (Nsd fewer) and a
# 3.2us FFT + GI; HE/EHT use a 4x denser FFT and a 12.8us FFT + GI. Centralizes
# the ht_vht vs he_eht split so it isn't repeated at every call site.
set_gen_profile() {
	local gen="$1"
	nsd_table=()
	tsym_table=()
	case "$gen" in
	ht_vht)
		nsd_table=([20]=52 [40]=108 [80]=234 [160]=468)
		tsym_table=([long]=4.0 [short]=3.6)
		width_options=(20 40 80 160)
		gi_options=(long short)
		;;
	he_eht)
		nsd_table=([20]=234 [40]=468 [80]=980 [160]=1960 [320]=3920)
		tsym_table=([0.8]=13.6 [1.6]=14.4 [3.2]=16.0)
		width_options=(20 40 80 160 320)
		gi_options=(0.8 1.6 3.2)
		;;
	esac
}

calc_rate() {
	local mcs="$1" nss="$2" width="$3" gi="$4" nsd tsym bpscr
	nsd="${nsd_table[$width]:-}"
	[ -n "$nsd" ] || { echo ""; return; }
	bpscr="${BPSCR[$mcs]:-}"
	[ -n "$bpscr" ] || { echo ""; return; }
	tsym="${tsym_table[$gi]:-}"
	[ -n "$tsym" ] || { echo ""; return; }
	awk -v nss="$nss" -v nsd="$nsd" -v bpscr="$bpscr" -v tsym="$tsym" \
		'BEGIN { printf "%.1f", nss * (nsd * bpscr) / tsym }'
}

# rate_vs_base RATE BASE -> "RATE Mbps (xN.NN)"
rate_vs_base() {
	awk -v rate="$1" -v base="$2" 'BEGIN { printf "%.1f Mbps (x%.2f)", rate, rate / base }'
}

# extract_field PATTERN TEXT -> the number captured by a "LABEL <N>" style token
extract_field() {
	echo "$2" | grep -oE "$1" | grep -oE '[0-9]+'
}

# parse_link RATE_LINE -> sets standard/gen/mcs/nss/gi_idx/is_eht from the raw
# `iw` tx-bitrate line. gen="legacy" means no HT/VHT/HE/EHT knobs apply.
parse_link() {
	local rate_line="$1" mcs_raw
	is_eht="false"
	if echo "$rate_line" | grep -q "EHT-MCS"; then
		standard="EHT (802.11be, Wi-Fi 7)"
		gen="he_eht"
		is_eht="true"
		mcs=$(extract_field 'EHT-MCS [0-9]+' "$rate_line")
		nss=$(extract_field 'EHT-NSS [0-9]+' "$rate_line")
		gi_idx=$(extract_field 'EHT-GI [0-9]+' "$rate_line")
	elif echo "$rate_line" | grep -q "HE-MCS"; then
		standard="HE (802.11ax, Wi-Fi 6)"
		gen="he_eht"
		mcs=$(extract_field 'HE-MCS [0-9]+' "$rate_line")
		nss=$(extract_field 'HE-NSS [0-9]+' "$rate_line")
		gi_idx=$(extract_field 'HE-GI [0-9]+' "$rate_line")
	elif echo "$rate_line" | grep -q "VHT-MCS"; then
		standard="VHT (802.11ac, Wi-Fi 5)"
		gen="ht_vht"
		mcs=$(extract_field 'VHT-MCS [0-9]+' "$rate_line")
		nss=$(extract_field 'VHT-NSS [0-9]+' "$rate_line")
	elif echo "$rate_line" | grep -qE '(^|[^A-Z-])MCS [0-9]+'; then
		standard="HT (802.11n, Wi-Fi 4)"
		gen="ht_vht"
		mcs_raw=$(extract_field 'MCS [0-9]+' "$rate_line")
		# HT reports one combined 0-31 index: per-stream MCS and NSS packed together.
		mcs=$((mcs_raw % 8))
		nss=$((mcs_raw / 8 + 1))
	else
		standard="Legacy (802.11a/b/g)"
		gen="legacy"
	fi
}

# print_breakdown IFACE RATE_LINE LIVE_MBPS WIDTH GI GI_UNIT BASE -> the raw
# output plus every intermediate value used to derive BASE.
print_breakdown() {
	local iface="$1" rate_line="$2" live_mbps="$3" width="$4" gi="$5" gi_unit="$6" base="$7"
	local nsd="${nsd_table[$width]}" tsym="${tsym_table[$gi]}"
	echo "raw iw output: ${rate_line# }"
	echo
	echo "${iface}: ${standard} MCS ${mcs}, NSS ${nss}, ${width}MHz, GI ${gi}${gi_unit} (live reported: ${live_mbps:-unknown} Mbps)"
	echo "  MCS ${mcs} -> ${MOD_NAME[$mcs]} (${MOD_BITS[$mcs]} bits/symbol) x code rate ${CR_LABEL[$mcs]} = ${BPSCR[$mcs]} bits/subcarrier"
	echo "  width ${width}MHz + ${standard} tone plan (${gen}) -> Nsd ${nsd} data subcarriers"
	echo "  GI ${gi} + ${standard} symbol timing (${gen}) -> Tsym ${tsym}us (FFT + guard interval)"
	echo "  rate = NSS(${nss}) * (Nsd(${nsd}) * bits/subcarrier(${BPSCR[$mcs]})) / Tsym(${tsym}us) = ${base} Mbps"
	echo
	echo "Theoretical PHY rate: ${base} Mbps"
}

# print_knob_table MCS NSS WIDTH GI BASE MAX_MCS -> one line per alternative
# GI/width/NSS/MCS, changing exactly one knob at a time from BASE.
print_knob_table() {
	local mcs="$1" nss="$2" width="$3" gi="$4" base="$5" max_mcs="$6" g w n m r

	echo
	echo "Per-knob multiplier from this baseline (one change at a time):"

	for g in "${gi_options[@]}"; do
		[ "$g" = "$gi" ] && continue
		r=$(calc_rate "$mcs" "$nss" "$width" "$g")
		[ -n "$r" ] && echo "  GI ${gi} -> ${g}:      $(rate_vs_base "$r" "$base")"
	done

	for w in "${width_options[@]}"; do
		[ "$w" -eq "$width" ] && continue
		r=$(calc_rate "$mcs" "$nss" "$w" "$gi")
		[ -n "$r" ] && echo "  width ${width}MHz -> ${w}MHz: $(rate_vs_base "$r" "$base")"
	done

	for n in 1 2 3 4; do
		[ "$n" -eq "$nss" ] && continue
		r=$(calc_rate "$mcs" "$n" "$width" "$gi")
		[ -n "$r" ] && echo "  NSS ${nss} -> ${n}:          $(rate_vs_base "$r" "$base")"
	done

	for m in $(seq 0 "$max_mcs"); do
		[ "$m" -eq "$mcs" ] && continue
		[ "$m" -eq 9 ] && [ "$width" -eq 20 ] && [ "$gen" = "ht_vht" ] && continue # MCS9 invalid at 20MHz VHT
		r=$(calc_rate "$m" "$nss" "$width" "$gi")
		[ -n "$r" ] && echo "  MCS ${mcs} -> ${m}:          $(rate_vs_base "$r" "$base")"
	done
}

iface="${1:-}"
if [ -z "$iface" ] || [ ! -d "/sys/class/net/${iface}" ]; then
	echo "Usage: $0 <wifi-iface>" >&2
	exit 1
fi

link=$(iw dev "$iface" link 2>/dev/null)
if ! echo "$link" | grep -q "^Connected"; then
	echo "Error: ${iface} is not connected" >&2
	exit 1
fi

rate_line=$(echo "$link" | grep "tx bitrate" | head -1)
live_mbps=$(echo "$rate_line" | grep -oE '^[[:space:]]*tx bitrate: [0-9.]+' | grep -oE '[0-9.]+$')

parse_link "$rate_line"

if [ "$gen" = "legacy" ]; then
	echo "${iface}: ${standard} (live reported: ${live_mbps:-unknown} Mbps)"
	echo "Legacy rates are fixed per modulation; no HT/VHT/HE/EHT knobs apply."
	exit 0
fi

width=$(iw dev "$iface" info 2>/dev/null | grep -oE 'width: [0-9]+' | grep -oE '[0-9]+')

if [ -z "$mcs" ] || [ -z "$nss" ] || [ -z "$width" ]; then
	echo "Error: could not determine MCS/NSS/width from ${iface}'s current link" >&2
	exit 1
fi

set_gen_profile "$gen"

# Resolve the guard interval actually in use.
if [ "$gen" = "he_eht" ]; then
	case "${gi_idx:-0}" in
	1) gi="1.6" ;; 2) gi="3.2" ;; *) gi="0.8" ;;
	esac
	gi_unit="us"
else
	gi="long"
	rate_short=$(calc_rate "$mcs" "$nss" "$width" "short")
	[ -n "$live_mbps" ] && [ "$live_mbps" = "$rate_short" ] && gi="short"
	gi_unit=""
fi

base=$(calc_rate "$mcs" "$nss" "$width" "$gi")

print_breakdown "$iface" "$rate_line" "$live_mbps" "$width" "$gi" "$gi_unit" "$base"

max_mcs=9
[ "$gen" = "he_eht" ] && max_mcs=11
[ "$is_eht" = "true" ] && max_mcs=13

print_knob_table "$mcs" "$nss" "$width" "$gi" "$base" "$max_mcs"
